...
function fixCommentsInFile(filePath) {
  ...
  // Replace lines that start with optional whitespace, then a single slash and a space, with //
  const fixed = content.replace(/^(\s*)\/ (.*)$/gm, '$1// $2');
  if (fixed !== content) {
...
console.log('Malformed comment fix complete!');
